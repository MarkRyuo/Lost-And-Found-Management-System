...
<body>
    <?php
        if (isset($_POST["submit"])) {
            ...
            if (empty($security) OR empty($email) OR empty($password) OR empty($repeatpassword)) {
            ...
            $sql = "SELECT * FROM tbl_security WHERE emailaddress = '$email'";
            ...
            if (count($errors) > 0 ) {
                ...
                $sql = "INSERT INTO tbl_security (security, emailaddress, password) VALUES (?, ?, ?)";
                $stmt = mysqli_stmt_init($conn);

                if (mysqli_stmt_prepare($stmt, $sql)) {
                    mysqli_stmt_bind_param($stmt, "sss", $security, $email, $passwordHash);
    ...
    <div class="container">
        ...
        <div class="student-register">Security Register</div>

        <form action="securityregister.php" method="post">
